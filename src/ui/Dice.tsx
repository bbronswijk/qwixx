...
export function Dice({ value, color, isRolling }: DiceProps) {
  ...
  return (
    <div className={cn("relative h-12 w-12 rounded-lg", bg, isRolling && "animate-spin blur-[2px] duration-200")}>
      {dotPositions[isRolling ? 6 : value]?.map((position, index) => <div key={index} className={cn("absolute h-2 w-2 rounded-full", dots, positionClasses[position])} />)}
    </div>
  );
}
