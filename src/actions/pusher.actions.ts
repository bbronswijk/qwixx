...
export async function notifyImmediateEndOfGameAction(variant: Variant, gamePin: number) {
  ...
  await pusher.trigger(`presence-${gamePin}-${variant}`, PusherEvent.endGame, {
    ended: true,
  });
}

export async function notifyScoreSharedAction(variant: Variant, pin: number, store: State, score: number, nickname: string) {
  await Promise.all([endGameAction(pin), saveScore(pin, nickname, score, store)]);

  await pusher.trigger(`presence-${pin}-${variant}`, PusherEvent.shareScore, {
    nickname,
    score,
  });
}
